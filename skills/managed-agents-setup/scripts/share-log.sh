...
# Use when: install fails and user wants Luke to look at it.
#
# Output: short ID (e.g. "abc123") that maps to a private gist URL.
# Support: Luke pastes the ID into `get-support-log.sh abc123` which resolves to the gist.
set -euo pipefail
...
Send the short ID to Luke to get support.
EOF
