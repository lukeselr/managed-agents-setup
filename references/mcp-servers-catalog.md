...
|----------|---------|------|-------|
| GoHighLevel | `https://mcp.gohighlevel.com` | static_bearer (PIT token) | Community + official MCPs available. PIT token scoped to location. |
...
| n8n | `https://<your-n8n-domain>/mcp` | static_bearer (n8n API key) | User's own n8n instance. |
...
| **Composio (static) — headless** | `https://backend.composio.dev/v3/mcp` | static_bearer (`COMPOSIO_API_KEY`) | Same 500+ app gateway but using a Composio API key in `x-api-key` header. **Best for cron jobs and headless server agents** — no interactive consent needed. Requires per-user entity setup in Composio dashboard. |
...
3. **Provider not in Rube catalog**: niche or self-hosted services (your n8n, Hubstaff, custom webhook receivers) have no Rube route — use direct.
