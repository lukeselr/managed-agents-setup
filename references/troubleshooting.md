...
```bash
ssh <YOUR_SERVER_USER>@<YOUR_SERVER_IP> 'systemctl --user restart webhook-server'
```

### `/hook/<name>` returns 401
`X-Webhook-Secret` header missing or wrong. Fetch current value:
```bash
ssh <YOUR_SERVER_USER>@<YOUR_SERVER_IP> 'grep WEBHOOK_SECRET ~/agents-cc/shared/secrets.env'
```
