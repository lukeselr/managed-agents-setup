...
# Check the configured server first, then generic ~/agents-cc locally
if [ -f "$HOME/agents-cc/shared/secrets.env" ]; then
  ...
  ( ssh -o BatchMode=yes -o ConnectTimeout=3 -o StrictHostKeyChecking=no \
      -i ~/.ssh/clawd-key.pem <YOUR_SERVER_USER>@<YOUR_SERVER_IP> \
  ...
  if wait $SSH_PID 2>/dev/null; then
    server_setup_detected="true"
    secrets_env_path="\"<YOUR_SERVER_USER>@<YOUR_SERVER_IP>:~/agents-cc/shared/secrets.env\""
  fi
